...
const ANGLES = {
  ...
  // ------------------------------------------------- spring cleaning (CNY)
  // Enabled after Search Console showed /cleaning/spring-cleaning/ taking 931
  // impressions in 28 days - 40% of all site impressions - at position 67.
  // Head terms ("spring cleaning services singapore") are unwinnable for now;
  // town-level intent is where this service can actually place.
  'spring-cleaning/ang-mo-kio': 'Ang Mo Kio is a town of long-tenured households in flats built in the 1970s, and that changes what a Chinese New Year reset actually involves. Forty years in the same flat produces storage that newer estates simply do not have: store rooms, tops of wardrobes, things kept because there was room to keep them. The decluttering half of the job is usually larger than the cleaning half here. The flats are compact, so the work is dense rather than spread out, and a single session normally covers the whole home.',
  'spring-cleaning/bedok': 'Which half of Bedok you live in decides the job. Chai Chee and Bedok South flats date to the 1970s and carry decades of accumulation behind the visible surfaces; Bedok Reservoir stock is younger and lighter work. The coastal position adds something specific for a festive clean: window grilles, sliding door tracks and metal fittings pick up salt-laden grime faster than inland estates, and those are exactly the surfaces guests see on the way in. Book three weeks out, because the east fills early.',
  'spring-cleaning/bukit-batok': 'Bukit Batok runs to 4-room, 5-room and executive flats, so a Chinese New Year reset here takes longer and costs more than the headline figures quoted against a 4-room national average. The town is also at the stage where 1980s kitchen and bathroom finishes are at end of life, and that sets a ceiling on what any clean can deliver: the flat will look considerably better, but it will not look new. Households here often end up weighing a spring clean against starting a renovation instead.',
  'spring-cleaning/bukit-merah': 'Bukit Merah needs earlier booking than almost anywhere. The Tiong Bahru conservation flats have original terrazzo, timber and curved plaster that need pH-neutral products and hand methods rather than a machine and a general kit, and finding a vendor briefed on that is hardest in the fortnight when everyone is stretched. The small 2-room and 3-room flats around Redhill and Telok Blangah are quick in floor area but often long-occupied and content-heavy, which is where the hours actually go.',
  'spring-cleaning/choa-chu-kang': 'In Choa Chu Kang the binding constraint before Chinese New Year is availability rather than price. The town is far enough from the centre that vendors fill their peak-fortnight days closer in before they will travel out, so a slot booked three weeks ahead is worth more than a discount negotiated one week ahead. Flats here are large, which lengthens the session, and the older Teck Whye blocks need noticeably more time on kitchen and bathroom surfaces than the rest of the estate.',
  'spring-cleaning/hougang': 'Hougang has a high share of executive maisonettes, and a festive clean in one is a bigger job than the flat type suggests. There is an internal staircase, stair balustrades and an upper-storey bathroom, none of which a quote given by flat type over the phone reliably includes. Ask for those to be named in the scope. It also means the vendor needs a longer slot, which is harder to find late, so book earlier than you would for a standard flat.',
  'spring-cleaning/jurong-west': 'Jurong West is large enough that its size is the Chinese New Year problem. Vendors book out precinct by precinct, so a crew with availability in Boon Lay does not help a household in Taman Jurong, and the travel between them is real. Ask where the cleaner is coming from before agreeing a date. Taman Jurong is also the oldest part of the town, dating to the 1970s, and those flats need the most time of anywhere in the estate.',
  'spring-cleaning/punggol': 'Punggol is the lightest spring clean of any town on this list, and it should be priced that way. Nothing here is old enough to have accumulated grout staining or decades of kitchen residue, so a Chinese New Year reset is genuinely a detailed tidy rather than a restoration. The clutter in a young-family estate is toys, storage and boxes rather than forty years of accumulation. If you are quoted mature-estate rates for a Punggol flat, ask what specifically justifies them.',
  'spring-cleaning/sengkang': 'Sengkang sits between the two patterns. Compassvale, Anchorvale and Fernvale are young enough that a festive clean is mostly decluttering and detailing, much like Punggol next door. Rivervale is the exception: those flats date to the late 1990s and are now old enough that original kitchen and bathroom surfaces take real time. The town is full of young families, so storage and toys are usually the larger half of the work, and clearing them before the crew arrives converts booked hours into actual cleaning.',
  'spring-cleaning/tampines': 'Tampines runs two very different festive cleans. In the mature core along Tampines Streets 11 to 91, flats are 1980s, large, and carry the accumulation to match, so a full reset is a long session and often a two-person team. In Tampines North the flats are recent and the work is light. The town is also big enough that a vendor working one end will decline the other in peak fortnight, so confirm they actually cover your street before fixing a date.',
  'spring-cleaning/woodlands': 'Distance decides everything in Woodlands during the Chinese New Year fortnight. Crews can fill a day centrally without the Causeway-side travel, so the north is where availability disappears first and returns last. Booking three to four weeks out is realistic rather than cautious. The flats are large, which lengthens the session, and most of the town is now past the twenty-five year mark, so original finishes need more attention than the 1990s build date suggests.',
  'spring-cleaning/yishun': 'The Home Improvement Programme shapes a festive clean in Yishun. Upgraded bathrooms commonly sit inside otherwise original 1980s flats, so the newest part of the home needs the least attention while kitchen surfaces, window tracks and original floor tiling need the most. A vendor who walks in, sees a modern bathroom and assumes the flat is newer than it is will under-scope the rest. The flats are compact, so a full reset still fits comfortably into one session.',
};
